...
- **进 `GameConfig`（平衡旋钮，仅限三类）**：①玩家行动的**扁平成本**与②**恢复速率**——开垦/招商/征兵的金、体力消耗，每月体力恢复；③**AI 难度旋钮**——简单标量的 AI 行为概率/速度（如 `aiLevelUpRate`），无公式身份、便于整体调难度并在测试注入。它们均为简单标量。
...
- 做：38 城多势力节点地图；每局由玩家选择一位君主，其余现存君主均由 AI 控制；经营（征兵/产金等）+ 回合制战棋 + 占领；占领全部城池即胜利；localStorage 存档。
- 战棋规模约束：**棋盘 32×32、双方单位数 ≤ 10/方**（`12-battle` 放宽，替代早期 ≤8×6/≤6 方的占位约束）。
- 不做（第一版）：科技树、武将养成、动画/音效、联机、复杂剧情。
