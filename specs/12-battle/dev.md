...
export function attackDamage(atkPower: number, defPower: number, attackerTroops: number, counterPct: number, targetTroops: number): number

// 经验（§6.7.4）：含等级差加成 + 击溃额外经验；返回新增经验
export function experienceGain(troopDelta: number, attackerLevel: number, targetLevel: number, routed: boolean): number
...
export function attackableTiles(battle: BattleState, map: BattleMap, from: Position, troopType: TroopType): Position[]
```
...
export function resolveCampaignOutcome(
  state: GameState, officerIds: readonly OfficerId[], targetCityId: CityId,
  provisions: number, attackerWins: boolean): GameState
