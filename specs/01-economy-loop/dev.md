...
### shared/rng.ts
```ts
export interface Rng { readonly seed: number }
...
export function canDevelop(
  state: GameState, cityId: CityId, officerId: OfficerId, kind: DevelopKind, config: GameConfig,
...
export function canApply(state: GameState, action: Action, config?: GameConfig): { ok: boolean; reason?: string }
```
