...
/**
 * 战斗分胜负后续跑月末：写回（concludeBattle）→ 移除已结算的 campaign → 继续处理剩余 campaign/尾段。
 * 要求 state.activeBattle 已有 outcome；否则 no-op。
 */
export function resumeMonth(state: GameState, config: GameConfig): GameState {
  ...
  const concluded = concludeBattle(state) // 写回 + 清空 activeBattle
  const dropped = dropFirstCampaign(concluded)
  return advanceCampaigns(dropped, config)
}

/** 逐条处理队列中的 campaign：玩家参与→挂起战斗返回；非玩家→速算并继续；无 campaign→尾段。 */
function advanceCampaigns(state: GameState, config: GameConfig): GameState {
  ...
  const dropped = { ...resolved, pendingCommands: resolved.pendingCommands.filter((_, i) => i !== idx) }
  return advanceCampaigns(dropped, config)
}

/** 出征攻或守一方为玩家君主即玩家参与（→交互式战斗）。 */
function isPlayerCampaign(state: GameState, c: CampaignPending): boolean {
  const attackerLord = state.officers[c.officerIds[0]!]?.lordId
  const defenderLord = state.cities[c.targetCityId]?.lordId
  return attackerLord === state.playerLordId || defenderLord === state.playerLordId
}
