...
describe('checkImmediateVictory 城池格 / 全灭', () => {
  ...
  it('玩家进攻单位进入城池格 → 玩家胜', () => {
    const b = makeBattle([
      unit('guanyu', 'player', city),
      unit('caocao', 'opponent', { x: 1, y: 1 }),
    ])
...
describe('reduceBattle endDay：扣粮 + 日界判负', () => {
  ...
  it('扣当日粮草后玩家方粮草归零 → 玩家败', () => {
    const b = makeBattle(
      [unit('guanyu', 'player', { x: 5, y: 5 }, 100), unit('caocao', 'opponent', { x: 20, y: 20 })],
      { playerProvisions: 2 }
    )
...
describe('canBattle 非法守卫', () => {
  ...
  it('操作对手方单位非法', () => {
    expect(canBattle(st, { type: 'act', officerId: 'caocao', terminal: { kind: 'rest' } }).ok).toBe(
      false
    )
  })
  it('攻击范围外目标非法', () => {
...
describe('canBattle 施法守卫', () => {
  ...
  it('禁咒状态不能施法', () => {
    const b = makeBattle([
      { ...unit('zhugeliang', 'player', { x: 5, y: 5 }), status: 'sealed' },
      unit('xunyu', 'opponent', { x: 6, y: 5 }),
    ])
    expect(
  ...
  it('MP 不足不能施放', () => {
    const b = makeBattle([
      { ...unit('zhugeliang', 'player', { x: 5, y: 5 }), mp: 5 },
      unit('xunyu', 'opponent', { x: 6, y: 5 }),
    ])
    expect(
      canBattle(withBattle(s, b), {
  ...
  it('混乱状态不能行动（含施法/普攻）', () => {
    const b = makeBattle([
      { ...unit('guanyu', 'player', { x: 5, y: 5 }), status: 'confused' },
      unit('caocao', 'opponent', { x: 6, y: 5 }),
    ])
    expect(
      canBattle(withBattle(s, b), {
...
describe('startDay 每日开头', () => {
  ...
  it('日界查粮草：玩家方粮草为 0 → 判败', () => {
    const b = makeBattle(
      [unit('guanyu', 'player', { x: 5, y: 5 }), unit('caocao', 'opponent', { x: 20, y: 20 })],
...
describe('concludeBattle 写回 + 组装 CampaignOutcome（攻方胜）', () => {
  ...
  it('占城：许昌归刘备；粮草覆盖式合并（双方剩余战场粮之和）；无条件战损', () => {
    ...
    expect(next.cities.xuchang!.food).toBe(120 + 300) // 覆盖式合并，非累加
    expect(next.cities.xuchang!.agriculture).toBe(Math.floor(350 * 0.95)) // 战损
  })
  it('activeBattle 清空', () => {
