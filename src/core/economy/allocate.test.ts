...
describe('canAllocate 前置校验', () => {
  it('满足条件时通过', () => {
    const s = withCity(createInitialState(1), 'chengdu', { reserveTroops: 200 })
    expect(canAllocate(s, 'chengdu', 'zhugeliang', 250).ok).toBe(true)
  })

  it('武将不在该城 -> 拒绝', () => {
    expect(canAllocate(createInitialState(1), 'chengdu', 'guanyu', 0).ok).toBe(false)
  })

  it('武将已占用 -> 拒绝', () => {
    const s = withOfficer(createInitialState(1), 'zhugeliang', { busy: true })
    expect(canAllocate(s, 'chengdu', 'zhugeliang', 50).ok).toBe(false)
  })

  it('amount < 0 -> 拒绝', () => {
    expect(canAllocate(createInitialState(1), 'chengdu', 'zhugeliang', -1).ok).toBe(false)
  })

  it('amount 超过带兵量上限 -> 拒绝', () => {
    const s = withCity(createInitialState(1), 'chengdu', { reserveTroops: 100000 })
    expect(canAllocate(s, 'chengdu', 'zhugeliang', 1601).ok).toBe(false)
  })

  it('amount 超过 后备兵+原兵 -> 拒绝', () => {
    const s = withCity(createInitialState(1), 'chengdu', { reserveTroops: 50 })
    // 后备兵 50 + 原兵 100 = 150
    expect(canAllocate(s, 'chengdu', 'zhugeliang', 151).ok).toBe(false)
  })
})
...
describe('allocate 分配（双向）', () => {
  it('N > 原兵：城 → 武将', () => {
    const s = withCity(createInitialState(1), 'chengdu', { reserveTroops: 200 })
    const next = allocate(s, 'chengdu', 'zhugeliang', 250)
  ...
  it('N < 原兵：上交回城', () => {
    ...
    const next = allocate(s, 'chengdu', 'zhugeliang', 40)
  ...
  it('N = 0：全部上交', () => {
    const s = withCity(createInitialState(1), 'chengdu', { reserveTroops: 0 })
    const next = allocate(s, 'chengdu', 'zhugeliang', 0)
  ...
  it('非法下令 no-op（返回原状态）', () => {
    const s = withOfficer(createInitialState(1), 'zhugeliang', { busy: true })
    expect(allocate(s, 'chengdu', 'zhugeliang', 50)).toBe(s)
  })
})
