...
describe('canDevelop 前置校验', () => {
  it('满足条件时通过', () => {
    ...
    expect(canDevelop(s, 'chengdu', 'zhugeliang', 'agriculture', cfg).ok).toBe(true)
  })

  it('武将不在该城 -> 拒绝', () => {
    const s = createInitialState(1)
    expect(canDevelop(s, 'chengdu', 'guanyu', 'agriculture', cfg).ok).toBe(false)
  })

  it('武将已占用 -> 拒绝', () => {
    const s = withOfficer(createInitialState(1), 'zhugeliang', { busy: true })
    expect(canDevelop(s, 'chengdu', 'zhugeliang', 'agriculture', cfg).ok).toBe(false)
  })
...
describe('develop 开垦/招商', () => {
  it('开垦增长农业（floor(智力/5)+[0,30]），扣金扣体力，占用武将', () => {
    const s = createInitialState(1)
    const next = develop(s, 'chengdu', 'zhugeliang', 'agriculture', cfg)
  ...
  it('招商增长商业', () => {
    const s = createInitialState(1)
    const next = develop(s, 'chengdu', 'zhugeliang', 'commerce', cfg)
  ...
  it('确定性：相同 seed 相同结果', () => {
    ...
    const a = develop(s, 'chengdu', 'zhugeliang', 'agriculture', cfg)
    const b = develop(s, 'chengdu', 'zhugeliang', 'agriculture', cfg)
  ...
  it('非法下令 no-op（返回原状态）', () => {
    const s = withOfficer(createInitialState(1), 'zhugeliang', { busy: true })
    expect(develop(s, 'chengdu', 'zhugeliang', 'agriculture', cfg)).toBe(s)
  })
})
