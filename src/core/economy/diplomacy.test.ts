...
import {
  canEntice, entice, executeEntice,
  canAlienate, alienate, executeAlienate,
  canInstigate, instigate, executeInstigate,
  canInduce, induce, executeInduce,
} from './diplomacy'
...
describe('canEntice / canAlienate（敌方在任非君主武将）', () => {
  ...
  it('执行人占用 / 体力不足 / 城金不足 -> 拒绝', () => {
    expect(canEntice(withOfficer(createInitialState(1), 'guanyu', { busy: true }), 'guanyu', 'xunyu', cfg).ok).toBe(false)
    expect(canEntice(withOfficer(createInitialState(1), 'guanyu', { stamina: 10 }), 'guanyu', 'xunyu', cfg).ok).toBe(false)
    expect(canEntice(withCity(createInitialState(1), 'jiangling', { gold: 0 }), 'guanyu', 'xunyu', cfg).ok).toBe(false)
  })
})
...
describe('下令（占人 + 入队 + 不动 RNG）', () => {
  it('entice：扣体力20/城金50、busy、入队、rng 不变', () => {
    ...
    expect(next.pendingCommands).toContainEqual({ type: 'entice', officerId: 'guanyu', targetOfficerId: 'xunyu' })
  ...
  it('induce：扣体力10/城金50（城池压制满足时）', () => {
    ...
    expect(next.pendingCommands).toContainEqual({ type: 'induce', officerId: 'guanyu', targetOfficerId: 'caocao' })
  })
  it('前置不满足 -> no-op', () => {
