...
function isEnemyServingNonLord(state: GameState, execLord: OfficerId | null, targetId: OfficerId): boolean {
...
function isInstigateTarget(state: GameState, execLord: OfficerId | null, targetId: OfficerId): boolean {
...
function checkExecutor(state: GameState, officerId: OfficerId, staminaCost: number, goldCost: number): CommandCheck {
...
// —— 招揽（Entice）——
export function canEntice(state: GameState, officerId: OfficerId, targetOfficerId: OfficerId, config: GameConfig): CommandCheck {
...
export function entice(state: GameState, officerId: OfficerId, targetOfficerId: OfficerId, config: GameConfig): GameState {
  if (!canEntice(state, officerId, targetOfficerId, config).ok) return state
  return enqueueDiplomacy(state, officerId, targetOfficerId, config.enticeStaminaCost, config.enticeGoldCost, 'entice')
}
 ...
 */
export function executeEntice(state: GameState, officerId: OfficerId, targetOfficerId: OfficerId): GameState {
  const officer = state.officers[officerId]
  ...
  const target = { ...state.officers[targetOfficerId]!, cityId: officer.cityId, lordId: officer.lordId, loyalty }
  return { ...state, rng: rng2, officers: { ...state.officers, [targetOfficerId]: target } }
}

// —— 离间（Alienate）——
export function canAlienate(state: GameState, officerId: OfficerId, targetOfficerId: OfficerId, config: GameConfig): CommandCheck {
...
export function alienate(state: GameState, officerId: OfficerId, targetOfficerId: OfficerId, config: GameConfig): GameState {
  if (!canAlienate(state, officerId, targetOfficerId, config).ok) return state
  return enqueueDiplomacy(state, officerId, targetOfficerId, config.alienateStaminaCost, config.alienateGoldCost, 'alienate')
}

/** 月末执行离间：三关（安全线50, ALIENATE_COEFF）；成功仅目标忠诚 −4（下限0），无成功 RNG。守卫同招揽。 */
export function executeAlienate(state: GameState, officerId: OfficerId, targetOfficerId: OfficerId): GameState {
  const officer = state.officers[officerId]
  if (!officer || !isEnemyServingNonLord(state, officer.lordId, targetOfficerId)) return state

  const [passed, rng] = runThreeGates(state, officerId, targetOfficerId, INTEL_SAFETY, ALIENATE_COEFF)
  if (!passed) return { ...state, rng }
...
// —— 策反（Instigate）——
export function canInstigate(state: GameState, officerId: OfficerId, targetOfficerId: OfficerId, config: GameConfig): CommandCheck {
  const base = checkExecutor(state, officerId, config.instigateStaminaCost, config.instigateGoldCost)
  if (!base.ok) return base
...
export function instigate(state: GameState, officerId: OfficerId, targetOfficerId: OfficerId, config: GameConfig): GameState {
  if (!canInstigate(state, officerId, targetOfficerId, config).ok) return state
  return enqueueDiplomacy(state, officerId, targetOfficerId, config.instigateStaminaCost, config.instigateGoldCost, 'instigate')
}
 ...
 */
export function executeInstigate(state: GameState, officerId: OfficerId, targetOfficerId: OfficerId): GameState {
  const officer = state.officers[officerId]
  if (!officer || !isInstigateTarget(state, officer.lordId, targetOfficerId)) return state

  const [passed, rng] = runThreeGates(state, officerId, targetOfficerId, INTEL_SAFETY, INSTIGATE_COEFF)
  if (!passed) return { ...state, rng }
  ...
  for (const o of Object.values(state.officers)) {
    if (o.cityId === cityId && o.lordId === oldLord) officers[o.id] = { ...o, lordId: targetOfficerId }
  }
  const cities = { ...state.cities, [cityId]: { ...state.cities[cityId]!, lordId: targetOfficerId } }
  return { ...state, rng, officers, cities }
}

// —— 劝降（Induce）——
export function canInduce(state: GameState, officerId: OfficerId, targetOfficerId: OfficerId, config: GameConfig): CommandCheck {
  ...
  if (target.lordId !== target.id || isCaptive(state, targetOfficerId)) return { ok: false, reason: '目标须为敌方君主' }
  if (target.lordId === officer.lordId) return { ok: false, reason: '不能对己方君主劝降' }
  if (citiesOfLord(state, officer.lordId!).length < citiesOfLord(state, target.id).length * INDUCE_CITY_RATIO)
    return { ok: false, reason: '城池压制不足（需达敌方两倍）' }
  return { ok: true }
}

/** 下令劝降：扣体力/城金、busy、入队 {induce}；不动 RNG。非法 no-op。 */
export function induce(state: GameState, officerId: OfficerId, targetOfficerId: OfficerId, config: GameConfig): GameState {
  if (!canInduce(state, officerId, targetOfficerId, config).ok) return state
  return enqueueDiplomacy(state, officerId, targetOfficerId, config.induceStaminaCost, config.induceGoldCost, 'induce')
}
 ...
 */
export function executeInduce(state: GameState, officerId: OfficerId, targetOfficerId: OfficerId): GameState {
  const officer = state.officers[officerId]
  if (!officer) return state
  const target = state.officers[targetOfficerId]
  if (!target || target.lordId !== target.id || target.lordId === officer.lordId || isCaptive(state, targetOfficerId)) return state
  ...
  // ② 城池压制重校
  if (citiesOfLord(state, execLord).length < citiesOfLord(state, targetLord).length * INDUCE_CITY_RATIO) return state
  ...
  const officers = { ...state.officers }
  for (const o of Object.values(state.officers)) {
    if (o.lordId === targetLord) officers[o.id] = { ...o, lordId: cityIdSet.has(o.cityId) ? execLord : null }
  }
